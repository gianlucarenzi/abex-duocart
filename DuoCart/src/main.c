...
static int mount_atr(char *filename)
{
	...
	if (f_read(&mountedATR->fil, &mountedATR->atrHeader, ATR_HEADER_SIZE, &br) != FR_OK ||
		br != ATR_HEADER_SIZE || mountedATR->atrHeader.signature != ATR_SIGNATURE)
	{
...
static int load_file(char *filename)
{
	...
	{
		// not a car/xex file - guess the type based on size
...
static void emulate_standard_8k(void)
{
	// 8k
	RD5_HIGH
	uint16_t addr;
...
static void emulate_standard_16k(void)
{
	// 16k
	RD4_HIGH
	RD5_HIGH
...
static void emulate_bounty_bob(void)
{
	// 40k
	RD4_HIGH
	RD5_HIGH
	...
	while (1)
	{
		...
		if (!(c & S4))
		{
			...
			else
			{
				...
				if (addr == 0x0FF6)      bankPtr1 = &cart_ram1[0x0000];
...
void emulate_atarimax_128k(void)
{
	// atarimax 128k
	RD5_HIGH
	RD4_LOW
...
static void emulate_williams(void)
{
	// williams 32k, 64k
	RD5_HIGH
	RD4_LOW
...
static void emulate_OSS_B(void)
{
	// OSS type B
	RD5_HIGH
	RD4_LOW
...
static void emulate_blizzard(void)
{
	//16k
	RD4_HIGH
	RD5_HIGH
...
int main(void)
{
	...
	while (1)
	{
		...
		if (cmd == CART_CMD_ROOT_DIR)
			curPath[0] = 0;
		// SEARCH str
		else
		if (cmd == CART_CMD_SEARCH)
		{
			char searchStr[32];
		...
		if (cmd == CART_CMD_READ_ATR_SECTOR)
		{
		...
		if (cmd == CART_CMD_WRITE_ATR_SECTOR)
		{
			//uint8_t device = cart_d5xx[0x00];
			GREEN_LED_OFF
			RED_LED_ON
		...
		if (cmd == CART_CMD_NO_CART)
			cartType = 0;
		// REBOOT TO CART
		else
		if (cmd == CART_CMD_ACTIVATE_CART)
		{
